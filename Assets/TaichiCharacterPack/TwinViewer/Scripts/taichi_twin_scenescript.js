...
function Update () {
	...
	if(
		!obj.animation.IsPlaying(curAnimName) && 
		animationPlayFlgOld
	){
		nowTime = 0;
		playFlg = true;
	...
	animationPlayFlgOld = obj.animation.IsPlaying(curAnimName);
...
*/
			...
			faceMat_L = Resources.Load("Arum_v0111/Materials/succubus_a_face_l", Material);
			faceMat_M = Resources.Load("Arum_v0111/Materials/succubus_a_face_m", Material);
			FacialTexListFile = settingFileDir+CharacterCode+"facial_texture_list_a";
...
*/
			...
			faceMat_L = Resources.Load("Asphodel_v0111/Materials/succubus_b_face_l", Material);
			faceMat_M = Resources.Load("Asphodel_v0111/Materials/succubus_b_face_m", Material);
...
function SetFacialTex(_i:int){
	...
	for each( var mat:Material in faceSM.renderer.sharedMaterials){
		if(mat){
		if(mat.name ==  matName){
	...
	if(curLOD != 0){
		...
		for each( var mat:Material in faceSM.renderer.sharedMaterials){
			if(mat.name == faceMat_M.name){
				faceSM.renderer.materials[i] = faceMat_M;
			}else if(mat.name == faceMat_L.name){
				faceSM.renderer.materials[i] = faceMat_L;
			}
			i++;
		}
	}
	...
	if(_name){
		...
		*/
		...
		for each(var animName : String in animationListAll) {
			...
			if(animObj)
			{
				var anim = animObj.animation.clip;
				obj.animation.AddClip(anim,animName);
			}
...
function playAnimation(){
	...
	if(animObj)
	{
		if(playOnceFlg)obj.animation.Play(curAnimName); 
	}
	else
	{
		SetNextModel(-1);
...
function SetBackGround(_name:String) {
	if(_name){
		...
		planeObj.renderer.material.mainTexture =  loaded;
